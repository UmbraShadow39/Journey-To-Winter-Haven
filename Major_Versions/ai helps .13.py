...
def check(prompt, options=None):
    ...
    while True:
        ...
        # ----------------------------------------------------
        # Dev shortcut: restart game
        # ----------------------------------------------------
        if cleaned == "q":
            raise RestartException

        # ----------------------------------------------------
        # Dev shortcut: quick combat
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # Normal story validation
        # ----------------------------------------------------
        if options is None or cleaned in options:
            return cleaned
...
def get_name_input(prompt="\nWhat is your name, adventurer?\n> "):
    """
    Safe name prompt that ignores 'm', supports 'monster' debug,
    and always returns a proper string name.
    """
    ...
    while True:
        ...
        # Allow 'm' as a VALID NAME
        if cleaned != "":
            return cleaned  # <-- name returned
...
def monster_select_menu():
    ...
    if choice in monster_map:
        print(f"⚔️ You selected: {monster_map[choice].__name__}")
        return monster_map[choice]()  # return actual monster instance
...
def full_defensive_block(attacker, defender):
    messages = [
        f"{attacker.name} swings hard, but {defender.name} blocks perfectly, taking no damage.",
        f"{defender.name} deflects the attack with flawless timing.",
        f"{attacker.name}'s strike is completely absorbed by {defender.name}'s guard.",
        f"{defender.name} pivots just in time, avoiding all damage.",
        f"{defender.name} parries sharply, nullifying {attacker.name}'s attack.",
        f"{attacker.name} lunges forward, but {defender.name} sidesteps cleanly.",
        f"{defender.name} reads the attack and blocks effortlessly.",
        f"{attacker.name} commits to a strike, but {defender.name} is already defending.",
        f"{attacker.name}'s attack glances off harmlessly as {defender.name} braces.",
        f"{defender.name} anticipates the move and shuts it down completely."
    ]
...
def partial_defensive_block(attacker, defender, reduced_amount):
    messages = [
        f"{attacker.name} charges in, but {defender.name} blocks part of the impact.",
        f"{defender.name} absorbs some of the blow, reducing the damage.",
        f"{defender.name} pivots away, lessening the force of {attacker.name}'s attack.",
        f"{attacker.name}'s strike connects, but {defender.name} softens the hit.",
        f"{defender.name} brings up a guard just in time to reduce the damage.",
        f"{attacker.name} attacks fiercely, but {defender.name} braces and lowers the impact.",
        f"{defender.name} steps back quickly, avoiding some—but not all—of the damage."
    ]
...
def slime_poison_spit(slime, hero):
    ...
    dmg_after_def = max(0, base_damage - hero.defence)

    # Immediate poison damage
    poison_damage = 2

    # Total damage this turn
    total = dmg_after_def + poison_damage
    hero.hp -= total
    hero.poison_skip_first_tick = True

    # Apply poison state (slime alive = poison active)
    hero.poison_active = True
    hero.poison_amount = poison_damage   # Always 2 from this slime

    print(wrap(
        f"{slime.name} spits corrosive slime! "
        f"You take {dmg_after_def} damage + {poison_damage} poison damage!"
    ))
    return total
...
def red_slime_fire_spit(slime, hero):
    ...
    # --------------------------------
    if not hasattr(hero, "fire_stacks"):
        hero.fire_stacks = 0

    # Max 2 stacks
    hero.fire_stacks = min(hero.fire_stacks + 1, 2)

    # Prevent burn on the SAME turn
...
class Creator:
    ...
    def apply_defence(self, damage, attacker=None, defence_break=False):
        ...
        # Defence reduction calculations
        ...
        if defence_break:
            ...
            # Defence-break ignores armour entirely:
            actual = damage
...
class Hero(Creator):
    ...
    def show_game_stats(self, enemy=None):
        # Use overlapping colored HP bar (hero)
        hero_bar = hp_bar(
            self.hp,
            self.max_hp,
            size=12,
        ...
        # --- HERO HEADER ---
        print(f"🧝 {self.name.title()}   ❤️ [{hero_bar}] {self.hp}/{self.max_hp}")

                # >>> BERSERK STATUS DISPLAY <<<
        if getattr(self, "berserk_active", False):
            print(f"🩸 BERSERK ACTIVE! (+{self.berserk_bonus} damage)")
        elif getattr(self, "berserk_pending", False):
            print("🩸 (Berserk building... waiting for vision to clear)")


        # --- PRIMARY STATS ---
        print(
            f"⭐ Lvl {self.level}   "
            f"⚔️ {self.min_atk}-{self.max_atk}   "
            f"🛡️ {self.defence}   "
        ...
        print(f"💥 Bonus Dmg:  {self.current_bonus_damage}")

        # --- XP + GOLD ---
        print(f"📘 XP {self.xp}/{self.xp_to_lvl}   💰 {self.gold}")

        # --- POTIONS ---
        if self.potions:
            pot_line = "  ".join(
                f"{ptype.title()} x{count}"
                for ptype, count in self.potions.items()
            )
            print(f"🧪 {pot_line}")
        ...
        if enemy:
            ...
            print(f"💚 {enemy.name.title()}")
            print(f"❤️ [{ebar}] {enemy.hp}/{enemy.max_hp}   ⚔️ {enemy.min_atk}-{enemy.max_atk}")
        ...
        if warrior_turn:
            ...
            if warrior.poison_active:
                if getattr(warrior, "poison_skip_first_tick", False):
                    ...
                    warrior.hp -= warrior.poison_amount
                    warrior.hp = max(0, warrior.hp)
                    print(wrap(
            ...
            # 2) APPLY FIRE DAMAGE
            # ==========================
            if getattr(warrior, "fire_stacks", 0) > 0:
            ...
            if not enemy.is_alive():
                ...
                if len(warrior.monster_essence) >= 3:
                    print("\n✨ You have collected three essences!")
...
def intro_story_inner(warrior):
    ...
    if winter_heaven_info == "no":
        ...
        if night_choice == "2":
            ...
            if accept_help == "no":
                print(wrap("You decline the help and the creature says very well. The banks" \
                " remain pretty steep for a while and there are some serious rappids " \
                "farther downstream. Good luck finding your way out"))
                accept_help_2 = check(
                        ...
                        if GAME_WARRIOR.hp > 0:
                            ...
                            if survival_roll >= 16:
                                ...
                                print(wrap(
                                    ...
                                    "You can see the waterfall, only a few hundred feet ahead. "
                                    "You lose your footing as the current overwhelms you again."
                                ))
                                ...
                                print("I think you have a pretty decent chance to win the monster" \
                                " tournament {GAME_WARRIOR.name}, but not in your current state." \
