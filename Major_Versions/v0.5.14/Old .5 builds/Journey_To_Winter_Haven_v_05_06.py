...
def rest_phase(hero):
    ...
    while True:
        ...
        if _stone:
            print(f"{option}) Use Waterlogged Stone ({_stone.stone_charges}/{_stone.stone_max_charges} charges) — restore AP")
...
def debug_menu(warrior, enemy=None):
    while True:
        ...
        print("12) Level Up")
        print("13) Grant Defence Break (skill rank +1)")
...
def _debug_loot_menu(warrior):
    ...
    ALL_LOOT = [
        ...
        ("15", "Charged Jagged Rock (accessory) — Flayed One",     "Flayed One"),
...
def clear_all_status_effects(hero):
    ...
    hero.acid_stacks = []
    hero.acid_defence_loss = 0
    hero.warrior_bleed_dots = []   # Goblin Warrior Savage Slash
...
def slime_poison_spit(slime, hero):
    ...
    roll = chimera_double(slime, random.randint(1 + b, 3 + b))
...
def red_slime_fire_spit(slime, hero):
    ...
    # --------------------------------
    normal_roll = chimera_double(slime, random.randint(slime.min_atk, slime.max_atk))
    ...
    # --------------------------------
    fire_damage = chimera_double(slime, random.randint(2 + b, 3 + b))
...
def goblin_cheap_shot(enemy, warrior):
    ...
    # Max-damage attack
    actual = chimera_double(enemy, enemy.max_atk + b)
...
def paralyzing_shot(enemy, warrior, paralyze_turns=1):
    ...
    roll = chimera_double(enemy, random.randint(enemy.min_atk + b, enemy.max_atk + b))
...
def imp_sneak_attack(enemy, warrior):
    ...
    damage = chimera_double(enemy, enemy.max_atk + b)
...
def wolf_pup_bite(enemy, warrior):
    ...
    roll = chimera_double(enemy, random.randint(2 + b, 5 + b))
    actual = warrior.apply_defence(roll, attacker=enemy)
    warrior.hp = max(0, warrior.hp - actual)


    # Bite bonus (ignores defence)
    bite_bonus = chimera_double(enemy, random.randint(1 + b, 5 + b))
...
def blinding_charge(self, hero):
    ...
    damage = chimera_double(self, random.randint(4, 8))
...
def impact_bite(enemy, hero):
    ...
    impact_roll = chimera_double(enemy, random.randint(4 + b, 6 + b))
    bite_roll = chimera_double(enemy, random.randint(2 + b, 4 + b))
...
def try_death_defier(hero, reason=""):
    ...
    if hero.death_defier and hero.death_defier_active and not hero.death_defier_used:
        ...
        print(wrap(f"💀✨ {dd_name} surges — you refuse to die!"))
        if reason:
...
def hydra_hatchling_acid_spit(enemy, warrior):
    ...
    # -----------------------------
    roll = chimera_double(enemy, random.randint(enemy.min_atk, enemy.max_atk))
...
def savage_slash(enemy, warrior):
    ...
    is_chimera  = getattr(enemy, "chimera_extra_turns", False)
    ...
    for _ in range(max_stacks):
        if len(warrior.warrior_bleed_dots) < 2:
...
def psychic_shred(enemy, warrior):
    ...
    Standard Flayed One:  25% ATK+DEF reduction, 2-turn duration
    Hardened Flayed One:  30% ATK+DEF reduction, 3-turn duration
    (hardened detected via enemy.level >= 2)

    Stacking: first application 25%/30%, second application while
    first is active refreshes AND stacks (50%/60% total).
    ...
    # Hardened variant uses 30% / 3 turns; base uses 25% / 2 turns
    # Chimera version: double % but NO stacking — one application only
    is_hardened = getattr(enemy, "level", 1) >= 2
    is_chimera  = getattr(enemy, "chimera_extra_turns", False)
    debuff_pct  = 0.30 if is_hardened else 0.25
    if is_chimera:
        debuff_pct = min(0.90, debuff_pct * 2)
    duration    = (3 if is_hardened else 2) + (1 if is_chimera else 0)
    ...
    current_pct = warrior.psychic_atk_debuff   # both stats always equal

    # Chimera: no stacking — always fresh application at doubled %
    if warrior.psychic_debuff_turns > 0 and not is_chimera:
        # Already debuffed — stack and refresh (standard/hardened only)
        new_pct = min(0.90, current_pct + debuff_pct)
        warrior.psychic_atk_debuff   = new_pct
        warrior.psychic_def_debuff   = new_pct
        warrior.psychic_debuff_turns = duration
        warrior.psychic_debuff_skip  = False
        print(wrap(
            f"🧠 The psychic wound deepens! Your ATK and DEF are now reduced "
            f"by {int(new_pct * 100)}% for {duration} turns!"
        ))
    else:
        # Fresh application (always for Chimera)
        ...
        print(wrap(
            ...
            f"starting next round for {duration} turns!{chimera_tag} (First Aid cannot treat psychic wounds.)"
        ))
...
def use_waterlogged_stone(warrior):
    ...
    show_health(warrior)
    return True   # costs the player's turn
...
def activate_death_defier(hero):
    ...
    dd_name = "River Spirit" if getattr(hero, "death_defier_river", False) else "Death Defier"
    print(f"({dd_name} is now active. AP remaining: {hero.ap})")
    return True
...
class Equipment:
    def __init__(
        ...
        atk_debuff=0.0,       # accessory proc: % ATK reduction on enemy hit (Charged Jagged Rock)
        def_debuff=0.0,       # accessory proc: % DEF reduction on enemy hit (Charged Jagged Rock)
        debuff_turns=0,       # accessory proc: how many turns the debuff lasts
    ...
    def stat_lines(self):
        ...
        if self.bleed_turns > 0:
            lines.append(f"  🩹 Bleed: 2 dmg/turn for {self.bleed_turns} turns")
...
CHARGED_JAGGED_ROCK_STATS = {
    "poor":     {"atk_debuff": 0.10, "def_debuff": 0.10},
    "normal":   {"atk_debuff": 0.15, "def_debuff": 0.15},
    "uncommon": {"atk_debuff": 0.20, "def_debuff": 0.20},
    "rare":     {"atk_debuff": 0.25, "def_debuff": 0.25},
    "epic":     {"atk_debuff": 0.30, "def_debuff": 0.30},
    "legendary":{"atk_debuff": 0.35, "def_debuff": 0.35},
    "mythril":  {"atk_debuff": 0.40, "def_debuff": 0.40},
}
...
GOBLIN_WAR_BLADE_STATS = {
    "poor":      {"atk_min": 2, "atk_max": 2, "bleed_turns": 0},
    "normal":    {"atk_min": 2, "atk_max": 2, "bleed_turns": 1},
    "uncommon":  {"atk_min": 3, "atk_max": 3, "bleed_turns": 2},
    "rare":      {"atk_min": 4, "atk_max": 4, "bleed_turns": 3},
    "epic":      {"atk_min": 5, "atk_max": 5, "bleed_turns": 4},
    "legendary": {"atk_min": 6, "atk_max": 6, "bleed_turns": 5},
    "mythril":   {"atk_min": 7, "atk_max": 7, "bleed_turns": 6},
}
...
JAVELINA_TUSK_STATS = {
    ...
    "mythril":  {"atk_min": 5, "atk_max": 6, "bleed_turns": 6},
}
...
#
# Lightrender  (One-Handed): +4 ATK, +2 DEF — balanced, keeps accessory slot free.
# Destiny Definer (Two-Handed): +5 ATK, +1 DEF — raw power, no room for accessories.
# ----------------------------------------------------------------
# ----------------------------------------------------------------
# Tainted Champion's Shield  (armor) — Patronus drop (evil path)
# Fixed stats — no rarity. The -2 max HP reflects the corruption.
...
def _make_weapon_core():
    """
    Called when the Fallen Warrior drops the Weapon Core.
    Fixed stats — no rarity roll. Player chooses One-Handed or Two-Handed form.
    One-Handed: +4 ATK, +2 DEF — balanced, keeps accessory slot free.
    Two-Handed: +5 ATK, +1 DEF — raw power, no room for accessories.
    The choice is permanent for this run.
    """
    print("\n" + "═" * 50)
    ...
    d = WEAPON_CORE_ONEHANDED_STATS
    o = WEAPON_CORE_TWOHANDED_STATS

    print(f"  1) Lightrender  — One-Handed Sword")
    print(f"       ⚔️  ATK +{d['atk_min']}   🛡️  DEF +{d['defence']}")
    print(f"       Balanced. Lets you keep an accessory equipped.")
    print()
    print(f"  2) Destiny Definer  — Two-Handed Sword")
    ...
    while True:
        ...
        if choice == "1":
            stats = d
            form_name = "Lightrender"
            print(wrap("\nThe cube flattens and elongates — Lightrender takes shape. "
                       "Its edge catches light and holds it, as if the blade remembers the sun."))
            break
        elif choice == "2":
            stats = o
            form_name = "Destiny Definer"
            print(wrap("\nThe cube unfolds into a massive two-handed sword — Destiny Definer. "
                       "The weight of it is immense. This blade does not just cut. It decides."))
            break
        else:
            print("Enter 1 or 2.")
...
def make_loot(monster_name, monster_level=1, round_num=0):
    ...
    table = {
        ...
        "Fallen Warrior": lambda: _make_weapon_core(),
        ...
        "Flayed One": lambda: Equipment(
            ...
            slot         = "accessory",
            rarity       = rarity,
            atk_debuff   = CHARGED_JAGGED_ROCK_STATS[rarity]["atk_debuff"],
            def_debuff   = CHARGED_JAGGED_ROCK_STATS[rarity]["def_debuff"],
        ),
        ...
        "Patronus": lambda: Equipment(
            name    = "Light Corrupter",
            slot    = "armor",
            rarity  = "legendary",
            defence = TAINTED_CHAMPIONS_SHIELD_STATS["defence"],
            max_hp  = TAINTED_CHAMPIONS_SHIELD_STATS["max_hp"],
        ),
...
class Hero(Creator):
    ...
    def show_combat_stats(self, enemy=None):
        ...
        if getattr(self, "bleed_turns", 0) > 0:
            print(f"🩸 Bleed: 2 dmg/tick  ({self.bleed_turns}T remaining)")
...
SKILL_DEFS = {
    ...
    "heal": {
        ...
        "rank_descs": {
            ...
            2: "Restore 15% max HP. Cures Blind and Poison.                   1 AP",
            3: "Restore 20% max HP. Cures Blind and Poison.                   2 AP",
            4: "Restore 25% max HP. Cures Blind, Poison, Paralyze, Burn.      2 AP",
            5: "Restore 40% max HP. Cures all status effects.                 3 AP",
        },
    },
    "war_cry": {
        "name": "War Cry",
        ...
        "rank_descs": {
            1: "+1 ATK for 3 turns.                                           1 AP",
            2: "+2 ATK for 3 turns.                                           1 AP",
            3: "+3 ATK for 3 turns.                                           2 AP",
            4: "+3 ATK for 4 turns.                                           2 AP",
            5: "+5 ATK for 3 turns (burst).                                   3 AP",
        },
    },
    "defence_break": {
        "name": "Defence Break",
...
# --- allow partial investment ---
        ...
        if upgraded:
            ...
            check_jack_of_all_trades(hero)  # award title if all 3 beginner skills now rank 1+
        elif to_invest > 0:
            cost = next_skill_cost(hero, key)
            bank = hero.skill_progress.get(key, 0)
...
def skill_menu(hero, enemy):
    while True:
        ...
        if hero.death_defier and not hero.death_defier_active and not hero.death_defier_used:
            ...
            if hero.ap < cost:
                dd_label = "River Spirit" if hero.death_defier_river else "Death Defier"
                label = f"{dd_label} (Cost {cost} AP) [Not enough AP]"
                fn = None
            else:
                label = f"{dd_label} (Cost {cost} AP)"
...
def primordial_surge(enemy, warrior):
    """
    Chimera's signature breath attack — Primordial Surge.
    Fires on rest turns only, not regular attack turns.
    3 charges max, no recharge. Each use permanently degrades player stats
    for this fight: -2 max ATK, -2 DEF, -5 max HP.
    Stats restore after combat (win or lose).
    Heavy physical hit on top of the degradation.

    Desperation trigger chance (checked before firing):
      76-100% HP: 50%
      51-75%  HP: 65%
      26-50%  HP: 80%
       0-25%  HP: 90%
    """
    if getattr(enemy, "primordial_charges", 0) <= 0:
        return False

    # Desperation-scaled trigger chance
    hp_pct = enemy.hp / enemy.max_hp if enemy.max_hp > 0 else 0
    if hp_pct > 0.75:
        chance = 0.50
    elif hp_pct > 0.50:
    ...
    if random.random() > chance:
        return False  # didn't trigger this rest turn

    # Fire the surge
    enemy.primordial_charges -= 1
    charges_left = enemy.primordial_charges
    ...
    # True damage breath attack — bypasses defence entirely
    roll   = random.randint(enemy.max_atk, enemy.max_atk + 4)
    actual = roll
    warrior.hp = max(0, warrior.hp - actual)
    monster_math_breakdown(enemy, warrior, roll, actual, tag="Primordial Surge (true dmg)")
    ...
    # Store original stats on first hit for post-combat restore
...
def chimera_combo_bonus(enemy, warrior, move_result):
    """
    Chimera combo system — fires after every borrowed special move.
    Adds the Chimera's own basic attack (through defence) on top.

    Borrowed moves already dealt their own physical damage — this is just
    the Chimera following through with its own weight behind every hit.
    """
    b = lvl_bonus(enemy)

    # Basic attack follow-through (defence applies)
    basic_roll = random.randint(enemy.min_atk + b, enemy.max_atk + b)
...
def chimera_special_dispatcher(enemy, warrior):
    ...
    AP cost = tier: tier1=1AP, tier2=2AP, tier3=3AP. Elemental strike=1AP.
    Tier 3 borrowed moves get +1 turn duration above hardened via
    chimera_extra_turns flag — Chimera sustains debuffs longer but hits
    no harder than the original creature (lab experiment, not the real thing).
    """
    tier_map = {
        enemy.chimera_tier1: 1,
        enemy.chimera_tier2: 2,
        enemy.chimera_tier3: 3,
        chimera_elemental_strike: 1,
    }
    ...
    # Escalating weights by turn count — organized chaos.
    # Early fight favors tier 1, mid favors tier 2, late favors tier 3.
    # Last used move gets reduced weight to encourage variety.
    # 20% chaos roll ignores escalation entirely — anything can happen.
    turn = getattr(enemy, "turns_survived", 0)
    last = getattr(enemy, "chimera_last_move", None)

    if random.random() < 0.20:
        # Chaos turn — equal weights, ignore escalation
        base_weights = {
            enemy.chimera_tier1: 3,
            enemy.chimera_tier2: 3,
            enemy.chimera_tier3: 3,
            chimera_elemental_strike: 3,
        }
    elif turn <= 3:
        # Early — favor tier 1
        base_weights = {
        ...
        # Late — favor tier 3 and elemental
        base_weights = {
            enemy.chimera_tier1: 1,
            enemy.chimera_tier2: 2,
            enemy.chimera_tier3: 5,
            chimera_elemental_strike: 3,
        }
    ...
    # Filter to affordable moves only
    affordable = [(m, base_weights[m]) for m in moves
                  if enemy.ap >= tier_map.get(m, 1)]
    if not affordable:
        return None

    moves_a, weights_a = zip(*affordable)
    chosen  = random.choices(moves_a, weights=list(weights_a), k=1)[0]
    tier    = tier_map.get(chosen, 1)
    ap_cost = tier

    # Pre-pay tier cost; move's own ap -= 1 fires too so restore 1 first
    enemy.ap -= (ap_cost - 1)

    # Track last used move for variety weighting next turn
    enemy.chimera_last_move = chosen

    # Duration bonus for tier 3 borrowed moves only
    enemy.chimera_extra_turns = (tier == 3 and chosen is not chimera_elemental_strike)

    if ap_cost > 1 or enemy.chimera_extra_turns:
        extra_msg = " (+1 turn duration)" if enemy.chimera_extra_turns else ""
        print(wrap(f"\u26a0\ufe0f The Young Chimera channels its power! ({ap_cost} AP){extra_msg}"))
    ...
    # Combo follow-through — basic attack + doubled move damage
    if chosen is not chimera_elemental_strike:
        chimera_combo_bonus(enemy, warrior, result)
...
class Young_Chimera(Monster):
    def __init__(self):
        ...
        self.chimera_used_special = False  # forces basic attack next turn after special
        self.chimera_last_move    = None   # tracks last move slot for variety weighting

        # Cycle tracking — one cycle = player turn + enemy turn both completed
        self.combat_cycles = 0

        # Primordial Surge — breath attack, rest-turn only
        self.primordial_charges   = 3      # hard limit, no recharge
        self.primordial_triggered = False  # flag so rest turn knows if it fired
...
def player_basic_attack(warrior, enemy, multiplier=1.0, use_accessory=False):
    ...
    # 5d) Accessory proc effects — psychic residue (Charged Jagged Rock)
    # Residue accumulates each hit — no duration, persists until fight ends.
    # Reduction = floor(base_stat * accumulated_pct).
    # Cap = round(base_stat * per_hit_pct) — can never reduce beyond one hit's worth.
    # ATK floor is 1. DEF can reach 0.
    if use_accessory and acc and actual > 0 and enemy.is_alive():
        atk_debuff = getattr(acc, "atk_debuff", 0.0)
        def_debuff = getattr(acc, "def_debuff", 0.0)
        if atk_debuff > 0:
            # Accumulate residue pool (stored as running percentage)
            enemy.psychic_atk_debuff = min(
                getattr(enemy, "psychic_atk_debuff", 0.0) + atk_debuff,
                1.0   # hard ceiling at 100% (will be capped by per-hit round below)
            )
            enemy.psychic_def_debuff = min(
                getattr(enemy, "psychic_def_debuff", 0.0) + def_debuff,
                1.0
            )

            atk_pool = enemy.psychic_atk_debuff
            def_pool = enemy.psychic_def_debuff

            # Cap: max reduction = round(base × per_hit_pct), minimum 1
            atk_cap = max(1, round(enemy.psychic_base_max_atk * atk_debuff))
            def_cap = max(1, round(enemy.psychic_base_defence  * def_debuff))

            # Current reduction = floor(base × accumulated_pool), capped
            atk_loss = min(math.floor(enemy.psychic_base_max_atk * atk_pool), atk_cap)
            def_loss = min(math.floor(enemy.psychic_base_defence  * def_pool), def_cap)

            # Apply — ATK floor 1, DEF floor 0
            new_max_atk = max(1, enemy.psychic_base_max_atk - atk_loss)
            new_min_atk = max(1, enemy.psychic_base_min_atk - atk_loss)
            new_defence = max(0, enemy.psychic_base_defence  - def_loss)

            enemy.min_atk = new_min_atk
            enemy.max_atk = new_max_atk
            enemy.defence = new_defence

            # Exposed state: if DEF is already 0 and cap is reached, push to -1
            # Enemy stays at -1 (no further reduction). Player deals +1 true damage while exposed.
            already_exposed = getattr(enemy, "psychic_exposed", False)
            if new_defence == 0 and def_loss >= def_cap and not already_exposed:
                enemy.defence = -1
                enemy.psychic_exposed = True
                print(wrap(f"💀 The psychic residue finds no armour left to erode — "
                           f"{enemy.display_name} is EXPOSED! Your attacks deal +1 damage!"))
            elif already_exposed:
                enemy.defence = -1   # keep pinned at -1

            # Feedback message
            atk_changed = atk_loss > 0
            def_changed = def_loss > 0 or already_exposed
            at_atk_cap  = atk_loss >= atk_cap
            at_def_cap  = def_loss >= def_cap or already_exposed

            if atk_changed or (def_changed and not already_exposed):
                atk_part = f"ATK {enemy.psychic_base_max_atk}→{new_max_atk}" if atk_changed else ""
                def_part = f"  DEF {enemy.psychic_base_defence}→{new_defence}" if (def_loss > 0 and not already_exposed) else ""
                cap_tag  = " (capped)" if (at_atk_cap and at_def_cap) else ""
                print(wrap(f"🔮 Psychic residue seeps in! {enemy.display_name} weakens! "
                           f"{atk_part}{def_part}{cap_tag}"))
            elif not atk_changed and not already_exposed:
                pct_so_far = int(atk_pool * 100)
                print(wrap(f"🔮 Psychic residue builds on {enemy.display_name}... "
                           f"({pct_so_far}% accumulated, not yet enough to weaken)"))
    ...
    if not use_accessory and weapon and actual > 0 and enemy.is_alive():
        ...
        # --- Goblin War Blade: scaling bleed ---
        # Bleed damage = half the attack roll rounded up, min 1.
        # Stored as a single warrior_bleed_dots-style stack on the enemy.
        # War Cry amplifies this naturally since it boosts ATK which raises the roll.
        ...
        if (war_blade_turns > 0 and actual > 0 and enemy.is_alive()
            ...
            bleed_dmg = max(1, math.ceil(roll / 2))
            ...
            print(wrap(f"🩸 The war blade opens a deep wound! "
                       f"{enemy.display_name} bleeds for {bleed_dmg} dmg/turn "
...
def chimera_fight(warrior):
    ...
    Loss (cycles < 3) → Regular defeat. Player proved nothing. Game over.

    Loss (cycles >= 3) → Player proved their worth. A good deity intervenes.
           Player escapes the arena. Chimera still alive.
    ...
    if result:
        # -----------------------------------------------
        # VICTORY — Chimera vanquished, Scale drops
        # -----------------------------------------------
        ...
        if scale:
            print(wrap(
                "\nA thick iridescent scale peels free from the chimera's flank "
                "and lands at your feet. It pulses faintly with residual energy."
            ))
        ...
        # -----------------------------------------------
        if cycles < 3:
            # Didn't survive 3 full cycles — no intervention
            print("\n" + "═" * 50)
            print("   💀 OVERWHELMED")
            print("═" * 50)
            print(wrap(
                "The Young Chimera stands over you, unimpressed. "
                "You never had a chance to show what you were made of."
            ))
            input("\nPress Enter to continue...")

        else:
            # Survived 3+ cycles — player proved their worth, deity intervenes
            ...
            print("═" * 50)
            print(wrap(
                "You fall. The world goes quiet. "
                "Then — warmth. Something ancient and steady reaches into the dark "
                "and pulls you back from the edge. Not to spare you the pain. "
                "To tell you it was watching. That you were worth watching."
            ))
            print()
            print(wrap(
                "When you open your eyes the Chimera is gone. "
                "The arena gates are open. You don't know how. "
                "You crawl through them before anyone notices you're still breathing."
            ))
            print()
            print(wrap(
                "The Chimera is still out there. Somewhere in the dark beyond the arena walls. "
                "That thought follows you like a second shadow."
            ))
            input("\nPress Enter to continue...")
            ...
            if warrior_turn:
                ...
                if not enemy.is_alive():
                    ...
                    if loot:
                        ...
                        # Weapon Core: offer immediate equip since there's no rest after the final fight
                        if "Weapon Core" in loot.name:
                            while True:
                                eq_choice = _real_input("\nEquip the Weapon Core now? (yes/no): ").strip().lower()
                                if eq_choice == "yes":
                                    equip_item(warrior, loot)
                                    break
                                elif eq_choice == "no":
                                    print(wrap("The Weapon Core rests in your bag, ready when you need it."))
                                    break
                                else:
                                    print("Enter yes or no.")
                        else:
                            print(wrap("\nThis item has been added to your bag. Equip it from Inventory & Equipment at the next rest."))
                ...
                # -----------------------------------------------
                    ...
                    if enemy.name in ("Flayed One", "Drowned One"):
                        ...
                        if warrior.is_alive() and monster_ai_check(enemy, turn_count):
                            log(f"  [ENEMY] {enemy.display_name} follows with special move")
                            enemy.special_move(enemy, warrior)
                            _stone_absorb_charge(warrior)
                    elif enemy.name == "Young Chimera":
                        # Strict alternation — special then rest, repeat.
                        # Rest always includes basic attack + 1 AP regen + primordial chance.
                        # Special turn: should_special chance applies — if it fails or AP is
                        # out, falls back to basic attack (no rest flag set, retries next turn).
                        if getattr(enemy, "chimera_used_special", False):
                            # Rest turn — basic attack, AP regen, primordial chance
                            regen = 1
                            enemy.ap = min(enemy.max_ap, enemy.ap + regen)
                            print(wrap(f"🌀 The Young Chimera gathers itself... (+{regen} AP)"))
                            log(f"  [ENEMY] Young Chimera rests — basic attack (+{regen} AP regen)")
                            enemy_attack(enemy, warrior)
                            enemy.chimera_used_special = False
                            # Primordial Surge check on rest turns
                            if warrior.is_alive() and getattr(enemy, "primordial_charges", 0) > 0:
                                primordial_surge(enemy, warrior)
                        elif enemy.ap > 0 and should_special and random.random() > 0.25:
                            # Special turn — AP available, chance passed, 75% not holding back
                            log(f"  [ENEMY] Young Chimera uses Special Move")
                            enemy.special_move(enemy, warrior)
                            _stone_absorb_charge(warrior)
                            enemy.chimera_used_special = True
                        else:
                            # No AP, chance failed, or 25% basic attack feint — retry special next turn
...
def arena_battle(warrior, rounds_to_win=5):
    ...
    try:
        ...
        if champion:
            warrior.achievements.add("champion_of_the_arena")
            ...
            # Check both equipped slot and bag in case player never equipped it
            core = warrior.equipment.get("weapon")
            if core and "Weapon Core" not in getattr(core, "name", ""):
                core = None
            if not core:
                core = next((i for i in getattr(warrior, "inventory", []) if "Weapon Core" in getattr(i, "name", "")), None)
            if core:
                print(wrap(
                ...
                if core in getattr(warrior, "inventory", []):
                    warrior.inventory.remove(core)
