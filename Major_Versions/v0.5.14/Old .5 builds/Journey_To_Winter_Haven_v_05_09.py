...
def check_berserk_trigger(warrior):
    ...
    if warrior.hp / warrior.max_hp > 0.20 and not warrior.berserk_active:
...
def fallen_warrior_moral_choice(warrior):
    ...
    while True:
        ...
        if choice == "1":
            ...
            # Find Lightrender or Destiny Definer — equipped or in bag
            core = warrior.equipment.get("weapon")
            if core and core.name not in ("Lightrender", "Destiny Definer"):
                core = None
            if not core:
                core = next(
            ...
            if core:
                old_name      = core.name
                is_equipped   = (warrior.equipment.get("weapon") is core)
                ...
                core.defence  -= def_loss
                core.name     = new_name
                core.rarity   = "legendary"

                # If equipped, also update warrior's live stats
                if is_equipped:
                    warrior.min_atk += atk_gain
                    warrior.max_atk += atk_gain
                    warrior.defence  = max(0, warrior.defence - def_loss)
                    warrior.equipment_bonus_damage = core.atk_min
