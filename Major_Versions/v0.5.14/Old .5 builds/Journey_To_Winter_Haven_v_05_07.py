...
def goblin_cheap_shot(enemy, warrior):
    ...
    When used by Young Chimera: blind reduced to 1 turn — max ATK hit still lands.
    """
    if enemy.ap <= 0:
        return None
    ...
    # Chimera borrows this move but isn't a goblin — blind is shorter
    if enemy.name == "Young Chimera":
        ...
        print("😵 You are briefly BLINDED! (1 turn)")
    else:
        warrior.blind_turns = 3
        warrior.blind_long  = True
        warrior.is_blinded  = True
    ...
    print(f"👁️ {enemy.display_name} strikes while you're vulnerable for {actual} MAX DAMAGE!")
    return actual
...
def devouring_bite(enemy, warrior):
    ...
    if heal > 0:
        ...
        print(f"🧶 {enemy.display_name} devours flesh and regains {gained} HP!")
    else:
...
def primordial_surge(enemy, warrior):
    ...
    # ATK roll / 2 rounded up — true damage, ignores defence
    b      = lvl_bonus(enemy)
    roll   = random.randint(enemy.min_atk + b, enemy.max_atk + b)
    actual = math.ceil(roll / 2)
    warrior.hp = max(0, warrior.hp - actual)
    monster_math_breakdown(enemy, warrior, roll, actual,
                           tag=f"Primordial Surge (roll {roll} ÷ 2 = {actual}, ignores DEF)")
...
class Patronus(Monster):
    ...
    Shield grants +6 DEF +6 HP during fight. Stripped by Beast Gods on
    Death Defier trigger — DEF drops to base 4, dramatic final window.
    ...
    def __init__(self):
        super().__init__(
            name    = "Patronus",
            hp      = 100 + Patronus.SHIELD_HP_BONUS,  # 106 effective
            min_atk = 7,
            max_atk = 12,
            gold    = 0,
            xp      = 0,
...
def fallen_warrior_moral_choice(warrior):
    """
    Fires immediately after the Fallen Warrior is defeated.

    The Fallen Warrior begs to have his essence crushed — to be set free.
    The Beast God overseers demand the essence be returned to them.

    Choice A — Crush the essence (good path) → chimera_fight()
    Choice B — Return the essence (evil path) → patronus_fight()
    ...
    print()
    print(wrap(
        "His chest stills. A shimmer rises from his body — dense, pulsing, "
    ...
    print(wrap(
        ...
        "pours down from the overseer's box like oil through stone."
    ))
    print()
    input("Press Enter...")
    print()
    ...
    print(wrap(
        ...
        "His loot is yours — every piece of it. Gold. Favour. "
        "Our blessing on every fight that follows.\""
    ))
    print()
    input("Press Enter...")
    print()
    ...
    while True:
        ...
        if choice == "1":
            ...
            print()
            print(wrap(
                "The gold lands at your feet before you can blink. "
                "More than you've ever seen. The crowd erupts."
            ))
            print()

            # --- Corrupt the Weapon Core ---
            # Find it whether equipped or in bag
            core = warrior.equipment.get("weapon")
            if core and "Weapon Core" not in getattr(core, "name", "") \
                    and core.name not in ("Lightrender", "Destiny Definer"):
                core = None
            if not core:
                core = next(
                    (i for i in getattr(warrior, "inventory", [])
                     if i.name in ("Lightrender", "Destiny Definer")),
                    None
                )

            if core:
                old_name = core.name
                # +2 ATK, -3 DEF (net negative on defence)
                core.atk_min += 2
                core.atk_max += 2
                core.defence  = max(-3, core.defence - 3)
                core.name     = f"Tainted {old_name}"
                core.rarity   = "legendary"

                input("Press Enter...")
                print()
                print(wrap(
                    "Then you feel it. A pulse from the weapon at your side — "
                    "or in your bag. The Weapon Core."
                ))
                print()
                print(wrap(
                    "The Beast Gods' favour doesn't come for free. "
                    "Something passes through the blade like a current. "
                    "The metal darkens at the edges. The balance shifts."
                ))
                print()
                print(wrap(
                    f"Your {old_name} has become the {core.name}."
                ))
                print(f"  ⚔️  ATK +{core.atk_min}   🛡️  DEF {core.defence}")
                print(wrap(
                    "Hungrier. Sharper. At a cost."
                ))
    ...
    Loss (cycles < 3) → Regular defeat. Player didn't last long enough.
           No intervention. Game over.

    Loss (cycles >= 3) → Player survived long enough to prove their worth.
           Divine intervention pulls them back from the edge.
    ...
    print("\n" + "═" * 50)
    print("   ⚠️  THE TRUE CHALLENGE REVEALS ITSELF")
    print("═" * 50)
...
def patronus_fight(warrior):
    ...
    Loss (cycles < 3) → Regular defeat. No intervention.

    Loss (cycles >= 3) → Beast Gods intervene on Patronus's behalf.
           Player escapes. No drop.
    ...
    print("\n" + "=" * 50)
    print("   ⚔️  THE PROTECTOR DESCENDS")
    print("=" * 50)
    print(wrap(patronus.spawn_flavour))
    input("\nPress Enter to face Patronus...")

    result = battle(warrior, patronus)

    # Always restore DEF reduced by Patronus Defence Break
    ...
    if result:
        # -----------------------------------------------
        # VICTORY — Death Defier fires, Beast Gods intervene
        # -----------------------------------------------
        ...
        print("=" * 50)
        print(wrap(
            "Patronus falls to one knee. For a moment the arena is completely silent. "
            "Then something shifts in the air — a pressure that has nothing to do with wind. "
            "The Beast Gods are watching."
        ))
        print()
        print(wrap(
            "He stands back up. Slower. Different. "
            "The shield on his arm begins to crack along its edges, the nano-tech "
            "unravelling in thin threads of light. "
            "The Beast Gods are done with him."
        ))
        print()
        print(wrap(
            "The shield falls to the sand between you. "
            "Patronus looks down at it, then at you. Something in his expression "
            "is not defeat. It is relief."
        ))
        print()
        print(wrap(
            "\"Go,\" he says. \"Finish what I couldn't.\""
        ))
        print()

        # Tainted Champion's Shield drop — corrupted by Beast Gods on exit
        shield = make_loot("Patronus", monster_level=5)
        if shield:
            print(wrap(
                "A breastplate lies where Patronus fell — dense, dark at the edges, "
                "the metal warped by whatever the Beast Gods pressed into it on the way out. "
                "The protection is real. So is the cost."
            ))
            print(f"\n\U0001f381 You found: {shield.short_label()}")
        ...
        if cycles < 3:
            ...
            # Survived 3+ cycles — Beast Gods intervene on Patronus's behalf
            warrior.story_flags.add("patronus_intervention")

            print("\n" + "=" * 50)
            print("   \U0001f40d BEAST GODS INTERVENE")
            print("=" * 50)
            print(wrap(
                "You fall. Patronus raises his shield to finish it. "
                "Then the air changes."
            ))
            print()
            print(wrap(
                "Something vast and cold moves through the arena. "
                "Not for you. For him. "
                "The Beast Gods have decided you're more useful alive — "
                "a lesson for next time, a harvest yet to come."
            ))
            print()
            print(wrap(
                "The arena gates open on their own. "
                "You drag yourself through them before you understand what just happened. "
                "Behind you, Patronus watches you go. He doesn't look surprised."
            ))
            print()
            print(wrap(
                "The shield is still out there. "
                "Someone will have to come back for it."
            ))
            input("\nPress Enter to continue...")
                ...
                if choice == stats_num:
                    ...
                    if _atk:
                        log_attack(warrior.name, enemy.display_name, _atk["roll"], _atk["actual"], _atk["blocked"],
                                   bonus_parts=_atk.get("bonus_parts"), effect_tag=_atk.get("elem_tag", ""), is_player=True)
    ...
    try:
        ...
        for round_num in range(1, rounds_to_win + 1):
            ...
            if result == "win":
                ...
                # Moral choice fires here — splits good/evil path
                fallen_warrior_moral_choice(warrior)
                break
