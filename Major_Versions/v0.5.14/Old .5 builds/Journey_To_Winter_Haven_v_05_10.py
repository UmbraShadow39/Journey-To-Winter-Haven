...
def clear_all_status_effects(hero):
    ...
    hero.bonus_action_used  = False  # resets each new fight
...
def devouring_bite(enemy, warrior):
    ...
    # Lifesteal heal: half the damage dealt (rounded down) — can overheal up to 1.5x max HP
    heal = actual // 2
    if heal > 0:
        if not hasattr(enemy, "max_overheal"):
            enemy.max_overheal = int(enemy.max_hp * 1.5)
        before = enemy.hp
        enemy.hp = min(enemy.max_overheal, enemy.hp + heal)
