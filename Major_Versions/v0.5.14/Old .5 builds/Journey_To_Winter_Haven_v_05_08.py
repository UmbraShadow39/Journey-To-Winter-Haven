...
def _cjr_absorb(warrior, enemy):
    """
    Called on every accessory hit that lands.
    Adds enemy.max_atk * debuff_pct to the pool, capped at max_charges * 10.
    Returns True if the charge count changed (triggers display).
    """
    rock = _cjr_rock(warrior)
    if not rock:
        return False

    pct        = getattr(rock, "atk_debuff", 0.0)
    max_ch     = getattr(rock, "max_charges", 1)
    pool_cap   = max_ch * _CJR_POOL_PER_CHARGE

    old_pool   = getattr(warrior, "cjr_pool", 0.0)
    gain       = enemy.max_atk * pct
...
def _cjr_sync_atk(warrior, charges):
    """Apply +charges ATK bonus on top of the rock's base (stored on warrior)."""
    rock = _cjr_rock(warrior)
    if not rock:
        return
    base_min = getattr(warrior, "cjr_base_min_atk", warrior.min_atk)
    base_max = getattr(warrior, "cjr_base_max_atk", warrior.max_atk)
    warrior.min_atk = base_min + charges
    warrior.max_atk = base_max + charges
...
def equip_item(hero, item):
    ...
    # Charged Jagged Rock: store base ATK and apply existing charges on equip
    if getattr(item, "name", "") == "Charged Jagged Rock":
        ...
        hero.min_atk += charges
        hero.max_atk += charges
        if charges > 0:
            print(wrap(f"⚡ The stone hums with stored energy — +{charges} ATK active"))
...
def unequip_item(hero, item):
    ...
    # Charged Jagged Rock: strip charge-based ATK bonus on unequip
    if getattr(item, "name", "") == "Charged Jagged Rock":
        charges = getattr(hero, "cjr_charges", 0)
        hero.min_atk = max(1, hero.min_atk - charges)
        hero.max_atk = max(hero.min_atk, hero.max_atk - charges)
...
def level_up_menu(hero):
    ...
    if getattr(hero, "debug_mode", False):
        ...
        stat_cap = max(hero.stat_points, 2 if hero.level == 5 else 1)
...
class Equipment:
    def __init__(
        ...
        max_charges=0,        # trinket: max psychic charges (Charged Jagged Rock)
...
CHARGED_JAGGED_ROCK_STATS = {
    "poor":     {"atk_debuff": 0.15, "def_debuff": 0.15, "turns": 2, "max_charges": 1},
    "normal":   {"atk_debuff": 0.20, "def_debuff": 0.20, "turns": 2, "max_charges": 2},
    "uncommon": {"atk_debuff": 0.25, "def_debuff": 0.25, "turns": 3, "max_charges": 3},
    "rare":     {"atk_debuff": 0.30, "def_debuff": 0.30, "turns": 3, "max_charges": 4},
    "epic":     {"atk_debuff": 0.35, "def_debuff": 0.35, "turns": 4, "max_charges": 5},
    "legendary":{"atk_debuff": 0.40, "def_debuff": 0.40, "turns": 4, "max_charges": 6},
    "mythril":  {"atk_debuff": 0.45, "def_debuff": 0.45, "turns": 5, "max_charges": 7},
}
...
def make_loot(monster_name, monster_level=1, round_num=0):
    ...
    table = {
        ...
        "Flayed One": lambda: Equipment(
            ...
            debuff_turns = CHARGED_JAGGED_ROCK_STATS[rarity]["turns"],
...
def player_basic_attack(warrior, enemy, multiplier=1.0, use_accessory=False):
    ...
    # 5d) Charged Jagged Rock — absorb psychic energy on ANY hit (trinket slot)
    # Pool fills based on enemy.max_atk * debuff_pct per hit, persists run-wide
    if actual > 0 and enemy.is_alive() and _cjr_rock(warrior):
        changed = _cjr_absorb(warrior, enemy)
        if changed:
            print(wrap(cjr_bar(warrior)))

    # 5e) Accessory proc effects — psychic residue debuff (Charged Jagged Rock)
    # Applies enemy stat reduction when using the accessory attack.
    # Residue accumulates each hit — turn-limited, resets on expiry.
    ...
    # CJR debuff fires on any hit — rock is in trinket slot so use_accessory is irrelevant
    _cjr = _cjr_rock(warrior)
    if actual > 0 and enemy.is_alive() and _cjr and getattr(_cjr, "atk_debuff", 0.0) > 0:
        acc          = _cjr
        atk_debuff   = acc.atk_debuff
        def_debuff   = acc.def_debuff
        debuff_turns = getattr(acc, "debuff_turns", 0)
        current_turns = getattr(enemy, "jagged_rock_turns", 0)
        enemy.jagged_rock_turns = max(current_turns, debuff_turns)
...
def fallen_warrior_moral_choice(warrior):
    ...
    """
    # Loot and XP first — conclusion of the battle before the scene begins
    loot = make_loot("Fallen Warrior", monster_level=5)
    if loot:
    ...
    while True:
        ...
        if choice == "1":
            ...
            if core:
                old_name = core.name
                is_two_handed = (old_name == "Destiny Definer")

                # Two-handed: +8 ATK, -3 DEF  |  One-handed: +6 ATK, -2 DEF
                if is_two_handed:
                    atk_gain  =  8
                    def_loss  =  3
                    new_name  = "Destiny Destroyer"
                else:
                    atk_gain  =  6
                    def_loss  =  2
                    new_name  = "Duskbringer"

                core.atk_min += atk_gain
                core.atk_max += atk_gain
                core.defence  = max(-def_loss, core.defence - def_loss)
                core.name     = new_name
...
def patronus_fight(warrior):
    ...
    print()
    print(wrap(
        "The voice hits like a physical thing — not from the overseers' box, "
        "but from the far end of the arena. Deep. Certain. Furious."
    ))
    print()
    input("Press Enter...")
    print()
        ...
        while warrior.is_alive() and enemy.is_alive():
            ...
            if warrior_turn:
                ...
                if not enemy.is_alive():
                    ...
                    # 1. LOOT DROP — skip for Fallen Warrior (handled after moral choice)
                    ...
                    # 2. Xp animation bar — skip for Fallen Warrior (handled after moral choice)
                    if enemy.name != "Fallen Warrior":
            ...
            # ---------------------------------------
                ...
                # Charged Jagged Rock debuff countdown — decrement and clear on expiry
                if getattr(enemy, "jagged_rock_turns", 0) > 0:
                    enemy.jagged_rock_turns -= 1
                    if enemy.jagged_rock_turns <= 0:
                        enemy.psychic_atk_debuff = 0.0
                        enemy.psychic_def_debuff = 0.0
                        enemy.psychic_exposed    = False
                        if hasattr(enemy, "psychic_base_min_atk"):
                            enemy.min_atk = enemy.psychic_base_min_atk
                            enemy.max_atk = enemy.psychic_base_max_atk
                            enemy.defence = enemy.psychic_base_defence
                        print(wrap(f"🔮 The psychic residue fades — {enemy.display_name} recovers!"))
    ...
    try:
        ...
        for round_num in range(1, rounds_to_win + 1):
            ...
            if result == "win":
                ...
                # Award Champion of the Arena here — before the moral choice
                warrior.achievements.add("champion_of_the_arena")
                warrior.titles.add("champion_of_the_arena")
                print("\n" + "=" * 45)
                print("🏆  TITLE UNLOCKED: Champion of the Arena!")
                print("=" * 45)
                print(wrap(
                    "You have defeated every challenger the arena could throw at you. "
