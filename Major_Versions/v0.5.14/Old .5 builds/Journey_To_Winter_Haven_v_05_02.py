...
CHARGED_JAGGED_ROCK_STATS = {
    "poor":     {"atk_debuff": 0.10, "def_debuff": 0.00, "debuff_turns": 1},
    "normal":   {"atk_debuff": 0.10, "def_debuff": 0.10, "debuff_turns": 2},
    "uncommon": {"atk_debuff": 0.15, "def_debuff": 0.15, "debuff_turns": 3},
    "rare":     {"atk_debuff": 0.20, "def_debuff": 0.20, "debuff_turns": 3},
    "epic":     {"atk_debuff": 0.25, "def_debuff": 0.25, "debuff_turns": 4},
    "legendary":{"atk_debuff": 0.30, "def_debuff": 0.30, "debuff_turns": 5},
}
...
def make_loot(monster_name, monster_level=1, round_num=0):
    ...
    table = {
        ...
        "Flayed One": lambda: Equipment(
            ...
            debuff_turns = CHARGED_JAGGED_ROCK_STATS[rarity]["debuff_turns"],
        ),
...
def player_basic_attack(warrior, enemy, multiplier=1.0, use_accessory=False):
    ...
    # 5d) Accessory proc effects — psychic debuff (Charged Jagged Rock)
    if use_accessory and acc and actual > 0 and enemy.is_alive():
        atk_debuff   = getattr(acc, "atk_debuff", 0.0)
        def_debuff   = getattr(acc, "def_debuff", 0.0)
        debuff_turns = getattr(acc, "debuff_turns", 0)
        if atk_debuff > 0 and debuff_turns > 0:
            # Initialise debuff tracking on enemy if needed
            if not hasattr(enemy, "psychic_atk_debuff"):
                enemy.psychic_atk_debuff   = 0.0
                enemy.psychic_def_debuff   = 0.0
                enemy.psychic_debuff_turns = 0
                enemy.psychic_base_min_atk = enemy.min_atk
                enemy.psychic_base_max_atk = enemy.max_atk
                enemy.psychic_base_defence = enemy.defence
            # Refresh duration (no stacking for player accessory)
            enemy.psychic_atk_debuff   = atk_debuff
            enemy.psychic_def_debuff   = def_debuff
            enemy.psychic_debuff_turns = debuff_turns
            # Apply to enemy stats
            enemy.min_atk  = max(1, int(enemy.psychic_base_min_atk * (1 - atk_debuff)))
            enemy.max_atk  = max(1, int(enemy.psychic_base_max_atk * (1 - atk_debuff)))
            enemy.defence  = max(0, int(enemy.psychic_base_defence * (1 - def_debuff)))
            tag = f"{int(atk_debuff*100)}% ATK" + (f"+DEF" if def_debuff > 0 else "")
            print(wrap(f"🔮 The jagged rock pulses! {enemy.display_name} feels their resolve crack! "
                       f"({tag} reduced for {debuff_turns} turn{'s' if debuff_turns != 1 else ''})"))
        ...
        while warrior.is_alive() and enemy.is_alive():
            ...
            # ---------------------------------------
                ...
                # -----------------------------------------------
                    ...
                    # Flayed One: always basic attacks, THEN 33% chance to also Psychic Shred
                    if enemy.name == "Flayed One":
                        log(f"  [ENEMY] {enemy.display_name} attacks")
                        enemy_attack(enemy, warrior)
                        if warrior.is_alive() and monster_ai_check(enemy, turn_count):
                            log(f"  [ENEMY] {enemy.display_name} follows with Psychic Shred")
                            enemy.special_move(enemy, warrior)
                    elif should_special:
...
⚙️ Version Journey To Winter Haven v0.5.02 (Journey_To_Winter_Haven_v_05_02.py)
...
⚙️ Version Journey To Winter Haven v0.5.01 (Journey_To_Winter_Haven_v_05_01.py)
