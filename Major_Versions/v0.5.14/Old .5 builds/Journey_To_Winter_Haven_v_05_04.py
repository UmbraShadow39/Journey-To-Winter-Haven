...
def savage_slash(enemy, warrior):
    ...
    1) Basic attack fires first (goes through defence normally) — handled
       by monster_ai_check before calling this.
    2) Immediate bonus damage = half the base attack roll, rounded down,
       bypasses defence entirely.
    3) Applies 2 bleed stacks to warrior — each 3-5 dmg/tick (4-6 hardened),
       skip=True so they start on the player's next turn.
       Hardened variant: +1 min/max dmg, +1 extra turn duration.
    """
    if enemy.ap <= 0:
        return None
    ...
    print("\n⚔️  The Goblin Warrior lets out a savage war cry and slashes deep!")

    # Bonus damage — half the raw attack roll, bypasses defence
    roll = random.randint(enemy.min_atk + b, enemy.max_atk + b)
    bonus = max(1, roll // 2)
    warrior.hp = max(0, warrior.hp - bonus)
    print(f"💥 Savage Slash: {bonus} bonus damage (ignores defence)!")

    # Bleed stacks
    dmg_min = 4 if is_hardened else 3
    dmg_max = 6 if is_hardened else 5
    turns   = 3 if is_hardened else 2
    ...
    for _ in range(2):
        if len(warrior.warrior_bleed_dots) < 2:
            warrior.warrior_bleed_dots.append({
                ...
                "dmg_max":    dmg_max,
                "turns_left": turns,
                "skip":       True,   # starts on player's next turn
            })
            stacks_applied += 1

    if stacks_applied > 0:
        variant = "hardened " if is_hardened else ""
        print(f"🩸 {stacks_applied} {variant}bleed stack(s) applied! "
              f"({dmg_min}-{dmg_max} dmg/tick for {turns} turns — starts your next turn)")
    else:
        print("🩸 Your wounds are already bleeding as much as they can!")
...
def psychic_shred(enemy, warrior):
    ...
    print(wrap(
        "\n🧠 The Flayed One tears at your mind — your body feels like it's "
...
def get_ap_inflation(warrior) -> int:
    ...
    Each stack adds +1 to all special move AP costs.
    Max 3 stacks = +3 inflation.
...
def check_drown_punishment(warrior):
    ...
    If the player cannot afford ANY special move at rank 1 (after inflation),
    deal flat true damage based on current stack count.

    Damage table (standard):   1 stack=2, 2 stacks=3, 3 stacks=4
    Damage table (hardened):   1 stack=3, 2 stacks=4, 3 stacks=5
    Uses max_ap as the affordability baseline — not current AP.
    """
    stacks = getattr(warrior, "drown_stacks", 0)
    if stacks <= 0:
        return

    # Cheapest possible move at rank 1 after inflation
    base_cheapest = 1   # rank 1 of any move costs 1 AP base
    inflated_cost = base_cheapest + stacks

    # Use max_ap as baseline — protects players who spent AP actively
    if warrior.max_ap >= inflated_cost:
        return   # player could afford it — no punishment
...
def psychic_drown(enemy, warrior):
    ...
    Punishment: if player's max_ap < cheapest inflated move cost,
    deal flat true damage (2/3/4 standard, 3/4/5 hardened) per stack count.
    Does NOT interact with First Aid ranks 1-5.
    TODO: Triage (First Aid Rank 6+) will cleanse psychic debuffs.
    """
    if enemy.ap <= 0:
    ...
    print(wrap(
        "\n💧 The Drowned One reaches into your mind — your lungs fill with "
    ...
    if current_stacks >= 3:
        ...
        print(wrap(
            f"💧 The drowning sensation intensifies but cannot get worse! "
        ...
        print(wrap(
            f"💧 Stack {warrior.drown_stacks}/3! All special move AP costs "
...
def _stone_absorb_charge(warrior):
    ...
    if trinket.stone_charges < trinket.stone_max_charges:
        ...
        print(wrap(
            f"\U0001faa8 The Waterlogged Stone pulses — it absorbs the psychic energy! "
...
def _clear_psychic_drown(warrior):
    """
    Zeroes all drown tracking fields.
...
class Equipment:
    ...
    def stat_lines(self):
        ...
        if self.paralyze_chance > 0:
            lines.append(f"  🧊 {int(self.paralyze_chance*100)}% chance to paralyze ({self.paralyze_turns} turns)")
...
PARALYZING_ARROW_STATS = {
    ...
    "mythril":  {"atk_min": 5, "atk_max": 6, "paralyze_chance": 1.00, "paralyze_turns": 6},
}
...
WEAPON_CORE_DEFENSIVE_STATS = {
    ...
    "mythril":  {"atk_min": 6, "atk_max": 6, "defence": 7},
}
...
WEAPON_CORE_OFFENSIVE_STATS = {
    ...
    "mythril":  {"atk_min": 11, "atk_max": 11, "defence": 4},
}
...
class Hero(Creator):
    ...
    def show_combat_stats(self, enemy=None):
        ...
        if getattr(self, "defence_broken", False) and getattr(self, "defence_break_turns", 0) > 0:
            print(f"🛡️  Defence Break: {self.defence_break_turns}T remaining")
    ...
    try:
        ...
        while warrior.is_alive() and enemy.is_alive():
            ...
            if warrior_turn:
                ...
                # ==========================
                # 5b) PSYCHIC DROWN PUNISHMENT
                # If drown stacks mean player can't afford ANY rank-1 move,
                # deal true damage before they pick their action.
                # ==========================
                if getattr(warrior, "drown_stacks", 0) > 0:
                    check_drown_punishment(warrior)
                    if not warrior.is_alive():
                        print("The drowning pressure claims you...")
                        log(f"  [DEATH] {warrior.name} killed by Psychic Drown punishment.")
                        log(f"  [RESULT] DEFEAT — {warrior.name} fell to Psychic Drown.")
                        while True:
                            view = input("\nWould you like to view your combat log? (yes/no): ").strip().lower()
                            if view == "yes":
                                view_combat_log()
