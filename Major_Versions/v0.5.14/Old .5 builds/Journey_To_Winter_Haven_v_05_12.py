...
def _cjr_absorb(warrior, enemy, actual_damage):
    ...
    # Apply per-charge enemy debuff (-1 ATK / -1 DEF per charge, steps up as charges increase)
    if new_charges != old_charges and enemy is not None and enemy.is_alive():
        _cjr_apply_enemy_debuff(enemy, new_charges)
...
def _cjr_apply_enemy_debuff(enemy, charges):
    """
    Apply cumulative -1 ATK / -1 DEF per charge to the current enemy.
    Steps up as charges cross thresholds — recalculates from base each time.
    ATK floor 1, DEF floor 0.
    """
    ...
    new_min = max(1, enemy.cjr_base_min_atk - charges)
    new_max = max(1, enemy.cjr_base_max_atk - charges)
    new_def = max(0, enemy.cjr_base_defence  - charges)
...
class Equipment:
    ...
    def stat_lines(self):
        ...
        if self.base_atk > 0:
            lines.append(f"  ⚡ +{self.base_atk} ATK (base), +1 ATK per charge (max {self.max_charges})")
            lines.append(f"  📉 Each charge: enemy -1 ATK/-1 DEF")
        return lines
...
CHARGED_JAGGED_ROCK_STATS = {
    ...
    # Each full charge: player +1 ATK (on top of base), current enemy -1 ATK/-1 DEF.
    # Resets at rest between rounds.
    "poor":      {"base_atk": 0, "max_charges": 3, "fill_rate": 0.10},
    "normal":    {"base_atk": 1, "max_charges": 4, "fill_rate": 0.15},
    "uncommon":  {"base_atk": 2, "max_charges": 5, "fill_rate": 0.20},
    "rare":      {"base_atk": 3, "max_charges": 6, "fill_rate": 0.25},
    "epic":      {"base_atk": 4, "max_charges": 7, "fill_rate": 0.30},
    "legendary": {"base_atk": 5, "max_charges": 8, "fill_rate": 0.35},
    "mythril":   {"base_atk": 6, "max_charges": 9, "fill_rate": 0.40},
}
...
def _make_weapon_core(corrupted=False):
    ...
    if corrupted:
        ...
        o_stats = {"atk_min": 10, "atk_max": 10, "defence": -4}
...
class Flayed_One(Monster):
    def __init__(self):
        ...
        self.flayed_pool     = 10.0   # start at 1 full charge (pool per charge = 10)
...
def fallen_warrior_moral_choice(warrior, fallen=None):
    ...
    print(wrap(
        ...
        "pours down from the overseer's box like water through stone."
    ))
    print()
    input("Press Enter...")
    print()
