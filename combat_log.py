...
def reset_run_stats():
    """
    Reset all run-wide accumulators back to zero. Called by the main loop
    when the player chooses "play again" so the next run doesn't inherit
    damage totals, fight counts, etc. from the previous one.
    """
...
def show_run_score(warrior_or_name=None, outcome="defeat"):
    """
    End-of-run score display. Delegates to score.show_run_score() for the
    full scoring system. Kept here as a thin wrapper for backwards-compat
    with older call sites that may pass just a name string.

    Preferred call:  show_run_score(warrior, outcome="chimera_victory")
    Legacy call:     show_run_score(warrior_name)  → falls back to stat dump
    """
    # Detect: warrior object (has attributes) vs bare name string
    is_warrior = hasattr(warrior_or_name, "potions") or hasattr(warrior_or_name, "titles")

    if is_warrior:
        try:
            from score import show_run_score as _score_show
            # v0.6.15 BUG FIX: previously this called _score_show() and then
            # `return`'d with no value — so the final_score that was computed
            # and displayed on screen got DROPPED before reaching the leaderboard.
            # Every defeat saved with score=0. Now we return the actual int.
            return _score_show(warrior_or_name, outcome=outcome)
        except ImportError:
            # Fall through to legacy stat dump
            warrior_or_name = getattr(warrior_or_name, "name", "Hero")

    # ---- Legacy stat-dump fallback ----
    name = warrior_or_name if isinstance(warrior_or_name, str) else "Hero"
    r = _run_stats
...
def get_run_stats():
    """Return a copy of the accumulated run stats for scoring."""
