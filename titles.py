...
TITLE_DISPLAY = {
    ...
    "true_jack_of_all_trades": "True Jack of All Trades",
...
def award_title_with_buff(hero, key):
    ...
    if "perm_special" in buffs:
        # Permanent adrenaline / special damage bonus
        hero.perm_special = getattr(hero, "perm_special", 0) + buffs["perm_special"]
        buff_lines.append(f"+{buffs['perm_special']} Adrenaline")
...
def check_true_jack_of_all_trades(hero):
    """
    Award the True Jack of All Trades title once the hero has rank 2 or higher
    in every one of the five core skills.

    This is the breadth capstone — it rewards spreading investment across
    ...
    all_skills = ["power_strike", "heal", "war_cry", "defence_break", "death_defier"]
    if not all(hero.skill_ranks.get(s, 0) >= 2 for s in all_skills):
        return

    print("\n" + "=" * 50)
    print("🏅  TITLE UNLOCKED: True Jack of All Trades!")
    print("=" * 50)
    print("You have walked every path of the warrior's craft.")
    print("Where others sought mastery in one, you sought breadth in all.")
    print("=" * 50)

    award_title_with_buff(hero, "true_jack_of_all_trades")
...
def check_skill_mastery(hero, skill_key):
    ...
    Mastery titles and their passive effects:
      ...
      death_defier  → Death's Apprentice   (passive: Death Defier costs -1 AP + psychic rebound — flagged)
    """
    MASTERY_MAP = {
